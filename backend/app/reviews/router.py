...
async def list_reviews(
    ...
    return await service.list_reviews_by_movie(sk_movie_id)
...
async def get_review(
    ...
    review = await service.get_review_by_id(id_review)
    if review is None:
        raise HTTPException(
...
async def update_review(
    ...
    updated_review = await service.update_review(id_review, review)
...
async def delete_review(
    ...
    review = await service.delete_review(id_review)
    if review is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Review não encontrada",
